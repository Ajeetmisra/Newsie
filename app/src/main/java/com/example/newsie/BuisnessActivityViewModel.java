...
public class BuisnessActivityViewModel extends ViewModel {


    ArrayList<News> arrayList = new ArrayList<News>();
    MutableLiveData<List<News>> mBuisness;


    public LiveData<List<News>> getbuisnessNEWS() {
        Log.i("ajeet", "getStates: method is called");
        if (mBuisness == null) {
            mBuisness = new MutableLiveData<List<News>>();
//             aynsta();
            BuisnessAsync task = new BuisnessAsync();
            task.execute("http://newsapi.org/v2/top-headlines?country=in&category=business&apiKey=48f6cf55f79244fea4818e3455a08f64\n");
...
//        mNews.setValue(arrayList);
    ...
    class BuisnessAsync extends AsyncTask<String, Void, ArrayList<News>> {
