...
public class TechnologyActivityViewModel extends ViewModel {
    ArrayList<News> arrayList = new ArrayList<News>();
    MutableLiveData<List<News>> mTechnology;


    public LiveData<List<News>> gettechnologyNews() {
        Log.i("ajeet", "getStates: method is called");
        if (mTechnology == null) {
            mTechnology = new MutableLiveData<List<News>>();
//             aynsta();
            TechnologyAsynctask task = new TechnologyAsynctask();

            task.execute("http://newsapi.org/v2/top-headlines?country=in&category=technology&apiKey=48f6cf55f79244fea4818e3455a08f64\n");
...
//        mNews.setValue(arrayList);
    ...
    class TechnologyAsynctask extends AsyncTask<String, Void, ArrayList<News>> {
