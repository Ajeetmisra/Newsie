...
public class EntertainmentActivityViewModel extends ViewModel {
    ArrayList<News> arrayList = new ArrayList<News>();
    MutableLiveData<List<News>> mEntertainment;


    public LiveData<List<News>> getEntertainmentNews() {
        Log.i("ajeet", "getStates: method is called");
        if (mEntertainment == null) {
            mEntertainment = new MutableLiveData<List<News>>();
//             aynsta();
            EntertainmentAsyncTask task = new EntertainmentAsyncTask();
            task.execute("http://newsapi.org/v2/top-headlines?country=in&category=entertainment&apiKey=48f6cf55f79244fea4818e3455a08f64\n");
...
//        mNews.setValue(arrayList);
    ...
    class EntertainmentAsyncTask extends AsyncTask<String, Void, ArrayList<News>> {
