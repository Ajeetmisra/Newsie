...
public class SportActivityViewModel extends ViewModel {
    ...
    MutableLiveData<List<News>> mSport;


    public LiveData<List<News>> getSportNews() {
        Log.i("ajeet", "getStates: method is called");
        if (mSport == null) {
            mSport = new MutableLiveData<List<News>>();
//             aynsta();
            SportAsynctask task = new SportAsynctask();
            task.execute("http://newsapi.org/v2/top-headlines?country=in&category=sports&apiKey=48f6cf55f79244fea4818e3455a08f64\n");
...
//        mNews.setValue(arrayList);
    ...
    class SportAsynctask extends AsyncTask<String, Void, ArrayList<News>> {
