...
public class MainActivitityViewModel extends ViewModel {
    ArrayList<News> arrayList = new ArrayList<News>();
    MutableLiveData<List<News>> mNews;


    public LiveData<List<News>> getStates() {
        Log.i("ajeet", "getStates: method is called");
        if (mNews == null) {
            mNews = new MutableLiveData<List<News>>();
//             aynsta();
            StatesAsyncTask task = new StatesAsyncTask();
            task.execute("http://newsapi.org/v2/top-headlines?country=in&apiKey=48f6cf55f79244fea4818e3455a08f64");
...
//        mNews.setValue(arrayList);
    ...
    class StatesAsyncTask extends AsyncTask<String, Void, ArrayList<News>> {
