...
public class HealthActivityViewModel extends ViewModel {

    ArrayList<News> arrayList = new ArrayList<News>();
    MutableLiveData<List<News>> mHealth;


    public LiveData<List<News>> getHealthNews() {
        Log.i("ajeet", "getStates: method is called");
        if (mHealth == null) {
            mHealth = new MutableLiveData<List<News>>();
//             aynsta();
            HealthAsyncTask task = new HealthAsyncTask();
            task.execute("http://newsapi.org/v2/top-headlines?country=in&category=health&apiKey=48f6cf55f79244fea4818e3455a08f64\n");
...
//        mNews.setValue(arrayList);
    ...
    class HealthAsyncTask extends AsyncTask<String, Void, ArrayList<News>> {
